...
import { PLACEMENT_POINTS } from '@/lib/championships'
...
export default function RankingExplainer() {
  return <aside className={styles.panel} aria-labelledby="ranking-explainer-title">
    ...
    <section className={styles.points}>
      ...
      <dl>{PLACEMENT_POINTS.map((points, index) => { const place = index + 1; return <div key={place}>
        <dt><span className={styles[`place${place}`]}>{place}º</span> lugar</dt><dd>{points}<small> pts</small></dd>
      </div> })}</dl>
      <p>Do 9º lugar em diante, a pontuação é zero.</p>
    </section>
    ...
    <div className={styles.rule}><Medal size={19} /><div><h4>Os últimos 5 campeonatos</h4><p>Da esquerda para a direita, do mais recente ao mais antigo. As cores mostram colocações do 1º ao 8º lugar. Cinza indica ausência, posição fora do top 8 ou uma etapa ainda não disponível.</p></div></div>
    <p className={styles.example}><strong>Por exemplo:</strong> um título + um 3º lugar = <b>350 pontos</b> na temporada.</p>
  </aside>
}
