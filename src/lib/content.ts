...
export async function getPublicContent(): Promise<ContentData> {
  ...
  try {
    ...
    const [heroQuery, gamesWithRecentFormQuery, eventsQuery, settingsQuery, photosQuery] = await Promise.all([
    ...
    const gallery = settings?.banner_image_url ? {
      ...
      photos: (photosQuery.data ?? []).map((photo: any) => ({ id: photo.id, src: displayMediaUrl(photo.image_url) as string, alt: photo.alt_text, downloadUrl: displayMediaUrl(photo.download_url || photo.image_url) as string })),
    } : undefined
    return {
